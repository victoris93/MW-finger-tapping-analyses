...
library(reshape2)
data.path="/Users/VictoriaShevchenko/Documents/STAGE_M2/Experiment/Analyses/data"
...
all_data_nback <- cbind(all_data_nback, intention_distraction_values)
...
get.nback_v <- function(d, nback=20, which.apen.m=3, on.task.crit=1){ #, on.task.crit=1
 # if( !("part" %in% names(d))){
 #   d$part=1 ## this is for pilot2
 # }
  
  d %>% filter(stimulus=="probe_task") %>%  #  |stimulus=="probe_intention" | stimulus=="probe_distraction") %>%
    ...
    do({
      ...
      probeix=which(dd$stimulus=="probe_task") #|dd$stimulus=="probe_intention" | dd$stimulus=="probe_distraction")
  ...
  d.nback %>% 
    group_by(condition, subj, probeix, focus,probe.response) %>% #focus,
    summarize(apen=apen_int(tap,3)[which.apen.m+1]) %>%
    mutate(log.apen=log(log(2)-apen))  %>% ungroup -> d.nback.apen
  
  
  d.nback %>% group_by(subj,focus,condition,probeix) %>% #focus,
...
all_data_nback <- get.nback_v(all_data, nback = 20, which.apen.m=2)
